...
legend(strcat('Num: ', num2str(length(X))),...
    'Equal Line',...
    ['linear fit:y=', num2str(linear_fit(1), '%.5f'), 'x+', num2str(linear_fit(2), '%.5f'),...
    newline, 'pearson coefficient:', num2str(r, '%.5f')]);
grid off;
...
legend(strcat('Diff num: ', num2str(length(X))),...
    strcat('Mean_Diff: ', num2str(mean_diff, '%.5f')),...
    strcat('Mean_Diff + 1.96 Std_Diff: ', num2str(mean_diff + 1.96 * std_diff, '%.5f')),...
    strcat('Mean_Diff - 1.96 Std_Diff: ', num2str(mean_diff - 1.96 * std_diff, '%.5f')),...
    ['linear_fit:y=', num2str(linear_fit(1), '%.5f'), 'x+', num2str(linear_fit(2), '%.5f')],...
    'Interpreter', 'none');
grid off;
